...
def process_vip_visit(data, user):
    """BR-042/043: Expedited VIP processing with optional approval bypass."""
    from .notifications import notify_vip_arrival
    ...
    if not visit.is_vip:
        visit.is_vip = True
        visit.save(update_fields=["is_vip"])
...
def assign_escort(data, user):
    ...
    visit = get_object_or_404(Visit, id=data["visit_id"])
    escort_staff = get_current_staff(user)
    ...
    VIPActivityLog.objects.create(
        ...
        details=f"Escort: {escort_staff}",
        recorded_by=escort_staff,
    )
...
def export_visitor_data(params, user):
    ...
    try:
        ...
        # BR-069: data transformation
        rows = []
        for v in visits_qs:
            rows.append({
                "visit_id": v.id,
                "visitor_name": v.visitor.full_name,
                "id_number": v.visitor.id_number,
                "id_type": v.visitor.id_type,
                "purpose": v.purpose,
        ...
        # BR-070 / BR-074: error handling + recovery
        op_log.status = DataOperationLog.STATUS_FAILED
        op_log.error_details = str(exc)
        op_log.completed_at = timezone.now()
        op_log.save()
        raise DataOperationError(f"Export failed: {exc}") from exc
...
def import_visitor_data(data_content: str, fmt: str, field_mapping: dict, user):
    ...
    try:
        ...
        op_log.completed_at = timezone.now()
        op_log.save()
        raise DataOperationError(f"Import failed and rolled back: {exc}") from exc
